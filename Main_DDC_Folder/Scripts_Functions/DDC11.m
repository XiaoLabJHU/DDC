...
counterbreak=0;
for test_prob=.01:.05:.99
    
    
    Surv_Blink_Dist=test_prob;
    Constant=Constantf;
    Order=Orderf;
    Timecc=Timeccf;
...
while steps<step_total
    ...
    [loc, fram]=Eliminate_Blinking_De_Loc14(LocalizationsFinalt,  Frame_Information, Surv_Blink_Dist, density, Constant, Resolution, A, Deviation_in_Probability, Traj_num);
    %[loc, fram]=Threshold_multi(LocalizationsFinal, Frame_Information, Thresh_Time, Thresh_dist, density, Constants, Resolution, Constants2);
    %     if length(Surv_Blink_Dist)>length(Surv_Blink_Distf)
    %         Surv_Blink_Distf=Surv_Blink_Dist;
    %     end
